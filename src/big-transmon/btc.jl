...
function build_prob(;evolution_time = 50.0, solver_type = altro, dt = 0.1, ω = 0.0, α = -2π * 0.143, levels = 10, a_max = 2π * 100e-3, sqrtbp = false, qs = [1e2, 1e0, 1e0, 1e-1] ,smoke_test=false,
    projected_newton = false, constraint_tol=1e-8, al_tol=1e-4,pn_steps=2, max_penalty=1e11, verbose=true, save=true, max_iterations=Int64(2e5),
    nf = false, nf_tol = 0., max_cost_value=1e8, benchmark=false, al_iters = 30)
    ...
    prob = Problem(model, obj, x0, evolution_time, xf=xf, X0=X0, U0=U0, constraints = cons)
